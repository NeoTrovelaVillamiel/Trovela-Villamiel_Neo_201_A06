...
[https://github.com/NeoTrovelaVillamiel/Trovela-Villamiel_Neo_201_A06](https://github.com/NeoTrovelaVillamiel/Trovela-Villamiel_Neo_201_A06)
...
names <- names(player_info)

for(player in player_info){
  ppg <- append(ppg, player[[4]]/player[[3]])
}
names(ppg) <- names(player_info)
...
ppg <- lapply(ppg, function(x)round(x,2))
...
rounding <- function(x){
  ...
  } else{ # other case is thousands
    return(round(x, -3))
  }
  
}

mat_round <- apply(mat_rand, MARGIN = 1:2, FUN = rounding)
...
for(month in colnames(max_temps)){
  if(month != 'state' && month != 'year'){
    averages <- append(averages, (mean(max_temps[[month]])))
  }
}

names(averages) <- c("jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec")
...
medians <- apply(max_temps[,3:14], 1, median)

#print(medians)

#print(medians[which.max(medians)])

#print(max_temps$year[which.max(medians)])

high_median_temp <- medians[[which.max(medians)]]
low_median_temp <- medians[which.min(medians)]
high_median_year <- max_temps$year[which.max(medians)]
low_median_year <- max_temps$year[which.min(medians)]

sprintf("The maximum median annual temperature of %.2f occured in %d and the minimum median annual temperature of %.2f occured in %d",high_median_temp, high_median_year, low_median_temp, low_median_year)
```
...
digits_sum <- function(n){
  if(n < 10){
    ...
    digit <- n %% 10
    #print(floor(n/10))
    return(digit + digits_sum(floor(n/10)))
  }
}

sum <- digits_sum(935068)
print(sum)
```
